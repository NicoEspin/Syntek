...
import { cn, formatBlogDate, isDesignCategory } from "@/lib/utils";

const transition = { duration: 0.7, ease: [0.16, 1, 0.3, 1] };

const badgeClasses = {
  primary: "border border-primary1/20 bg-primary1/92 text-black",
  violet: "border border-violet-hover/35 bg-violet-deep/92 text-on-violet",
};
...
export default function BlogPostCard({ post, variant = "grid", index = 0, locale: localeProp }) {
  ...
  const isDesignPost = isDesignCategory(post.category);
  const categoryBadgeClassName = isDesignPost ? badgeClasses.violet : badgeClasses.primary;
  const hoverBorderClassName = isDesignPost ? "hover:border-violet/35" : "hover:border-primary1/25";
  ...
  if (variant === "featured") {
    return (
      <Link
        href={href}
        data-cursor-zone
        data-cursor-accent={isDesignPost ? "violet" : "primary"}
        aria-label={`${post.title} - ${t("viewPost")}`}
      ...
      >
        <motion.article
          ...
          className={cn(
            "flex flex-col overflow-hidden rounded-3xl border border-white/8 bg-neutral-900/70 backdrop-blur-sm transition-colors duration-500 md:flex-row md:min-h-[22rem]",
            isDesignPost ? "hover:border-violet/35" : "hover:border-primary1/30"
          )}
        >
          ...
          <div className="flex flex-1 flex-col justify-center gap-5 p-6 md:p-10 lg:p-12">
            <div className="flex items-center gap-3 text-[10px] uppercase tracking-[0.22em]">
              ...
              <span
                className={cn(
                  "rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] backdrop-blur-sm",
            ...
            <h2
              className={cn(
                "max-w-lg text-[clamp(1.7rem,2.6vw,2.5rem)] font-extrabold leading-[1.08] tracking-tight text-white transition-colors duration-300",
                isDesignPost ? "group-hover:text-violet" : "group-hover:text-primary1"
              )}
            >
              {post.title}
            </h2>
            ...
            <div className="mt-2 flex flex-wrap items-center justify-between gap-4">
              ...
              <span
                className={cn(
                  "inline-flex items-center gap-2 text-xs uppercase tracking-[0.2em] text-white/55 transition-colors duration-300",
                  isDesignPost ? "group-hover:text-violet" : "group-hover:text-primary1"
                )}
              >
                {t("viewPost")}
  ...
  return (
    ...
    >
      <motion.article
        ...
        className={cn(
          "flex h-full flex-col overflow-hidden rounded-3xl border border-white/8 bg-neutral-900/60 backdrop-blur-sm transition-colors duration-500",
      ...
      >
        <div className="relative aspect-[16/10] overflow-hidden">
          ...
          <span
            className={cn(
              "absolute left-4 top-4 rounded-full px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.2em] backdrop-blur-sm",
              categoryBadgeClassName,
            )}
          >
            {categoryLabel}
        ...
        <div className="flex flex-1 flex-col gap-3 p-5">
          <h3
            className={cn(
              "text-lg font-semibold leading-snug tracking-tight text-white transition-colors duration-300",
              isDesignPost ? "group-hover:text-violet" : "group-hover:text-primary1"
            )}
          >
            {post.title}
