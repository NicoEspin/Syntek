...
export default function ActiveSystem({ copy, systemFlow, nodeStatus, compact = false, nodeLimit = 5 }) {
  const gridId = useId();
          ...
          {systemFlow.slice(0, nodeLimit).map((step, i) => {
            ...
            return (
              <div key={step} className="relative flex items-start gap-3 pb-5 pl-1 last:pb-0">
                ...
                <span className="relative z-[1] flex size-8 shrink-0 items-center justify-center rounded-full bg-[#A1E233] text-black">
  ...
  return (
    <div className="absolute inset-0">
      ...
      <svg viewBox={`0 0 ${VB_W} ${VB_H}`} className="absolute inset-0 h-full w-full" fill="none" preserveAspectRatio="none">
        ...
        <path d={flowPath()} stroke={`url(#flow-grad-${gridId})`} strokeWidth="1.5" strokeLinecap="round" />
      </svg>
      ...
      <div className="pointer-events-none absolute inset-0">
        {nodes.map((n, i) => {
          ...
          return (
            ...
            >
              <span className="flex size-6 items-center justify-center rounded-full bg-[#A1E233] text-black">
